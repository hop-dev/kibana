...
import { MatchedEntitiesPreviewRiskScoreRequestBody } from '../../../../../common/api/entity_analytics/risk_engine/preview_matched_entities_route.gen';
...
export const riskScoreMatchedEntitiesPreviewRoute = (
  ...
  router.versioned
    .post({
      access: 'internal',
      path: `${RISK_SCORE_PREVIEW_URL}/matched_entities`,
      security: {
        authz: {
    ...
    .addVersion(
      {
        ...
        validate: {
          request: {
            body: buildRouteValidationWithZod(MatchedEntitiesPreviewRiskScoreRequestBody),
          },
        },
      },
      async (
        ...
        request,
        response
      ): Promise<IKibanaResponse<MatchedEntitiesPreviewRiskScoreResponse>> => {
        ...
        try {
          ...
          const result = await riskScoreService.calculateScoresForMatchedUsers({
            matchedUsers: matchedEntities,
            afterKeys: {},
            debug,
            filter,
